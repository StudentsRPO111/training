...
Entity movement(char **arr, Entity player) {
	/*char xy;
	xy = _getch();*/
	if (!_kbhit())
		return player;
	...
	{
		case UP:
			if (arr[player.x ? player.x - 1 : SIZE - 1][player.y] == '?') break;
			player.x = player.x ? player.x - 1 : SIZE - 1;
			break;
		case DOWN:
			if (arr[(player.x + 1) % 20][player.y] == '?') break;
			player.x = (player.x + 1) % 20;
			break;
		case LEFT:
			if (arr[player.x][player.y ? player.y - 1 : SIZE - 1] == '?') break;
			player.y = player.y ? player.y - 1 : SIZE - 1;
			break;
		case RIGHT:
			if (arr[player.x][(player.y + 1) % 20] == '?') break;
...
Entity enemy_move(char **arr, Entity enemy) {
	...
	if (arr[(enemy.x + 1) % 20][enemy.y] == '?' || arr[enemy.x ? enemy.x - 1 : SIZE - 1][enemy.y] == '?') {
		arr[enemy.x][enemy.y] = ' ';
		enemy.y = enemy.y ? enemy.y + 1 : enemy.y - 1;
		arr[enemy.x][enemy.y] = '#';
	}
	else if (arr[enemy.x][(enemy.y + 1) % 20] == '?' || arr[enemy.x][enemy.y ? enemy.y - 1 : SIZE - 1] == '?') {
		arr[enemy.x][enemy.y] = ' ';
		enemy.x = enemy.x ? enemy.x + 1 : enemy.x - 1;
		arr[enemy.x][enemy.y] = '#';
	}
	else {
		arr[enemy.x][enemy.y] = ' ';
...
int main() {
	...
	cout << "\n\tХотите нарисовать карту? (yes/no)\n";
	cin >> yes;
	_strlwr(yes);
	...
	while (!check_kill(player, enemy1) && !check_kill(player, enemy2)) {
		...
		delay_time = clock();
		if (delay_step == 5) {
			if (delay_time % 600 >= 0 && delay_time % 600 <= 100) {
